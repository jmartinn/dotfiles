 ...
 *
 * Press F8 to start, press it again to stop.
 * Press F9 to cancel and discard the in-flight transcript.
 *
 * Focus-aware: F8/F9 are intercepted at the TUI input layer (before any
...
//      before the focused component, so F8 works even while a custom
...
export default function (pi: ExtensionAPI) {
  ...
  // Global input listener: catches F8/F9 before ANY focused component,
  ...
  const onGlobalInput = (data: string) => {
    ...
    // event type. Without this guard a single physical F8 press toggles
    ...
    if (matchesKey(data, "f8")) {
      dbg(`F8 (data=${JSON.stringify(data)}) state=${state}`);
      if (lastCtx) toggleDictation(lastCtx);
      return { consume: true };
    }
    if (matchesKey(data, "f9")) {
      dbg(`F9 (data=${JSON.stringify(data)}) state=${state}`);
      cancelDictation();
      return { consume: true };
    }
    return undefined;
